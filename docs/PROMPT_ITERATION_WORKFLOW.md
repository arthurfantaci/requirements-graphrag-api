...
2. Click on a prompt (e.g., `graphrag-intent-classifier`)
...
2. Name it with a version suffix: `graphrag-intent-classifier-v2`
...
client = Client()
definition = PROMPT_DEFINITIONS[PromptName.INTENT_CLASSIFIER]

# Push as variant
client.push_prompt(
    'graphrag-intent-classifier-v2',
    object=definition.template,
    description='Intent Classifier v2: Improved classification for edge cases',
...
|---------|---------|----------|
| `{name}-v2` | `graphrag-intent-classifier-v2` | Simple version increment |
| `{name}-{date}` | `graphrag-intent-classifier-20240119` | Date-based versioning |
| `{name}-{experiment}` | `graphrag-intent-classifier-detailed-reasoning` | Experiment name |
...
```bash
# Compare intent-classifier baseline vs v2 variant
uv run python scripts/run_prompt_comparison.py intent-classifier --variant v2
```
...
uv run python scripts/run_prompt_comparison.py intent-classifier --variant v2 --iterations 3
```

### Save Report to File

```bash
uv run python scripts/run_prompt_comparison.py intent-classifier --variant v2 -o comparison_report.json
```
...
winning_prompt = client.pull_prompt("graphrag-intent-classifier-v2")

# Push as the new baseline with production tag
client.push_prompt(
    "graphrag-intent-classifier:production",
...
```
Good:  graphrag-intent-classifier-detailed-reasoning-v1
Bad:   graphrag-intent-classifier-test
```
...
```
ERROR - Variant prompt not found: graphrag-intent-classifier-v2
```
...
| Compare prompts | `uv run python scripts/run_prompt_comparison.py intent-classifier --variant v2` |
