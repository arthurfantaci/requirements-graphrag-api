...
class TestCostTrackerResearch:
    """Verify cost tracking in Research subgraph nodes."""

    @pytest.mark.asyncio
    async def test_identify_entities_records_cost(self, mock_config):
        """identify_entities node should record cost with operation='entity_identification'."""
        ...
        entity_json = json.dumps(
            {
                "entities": [{"name": "DO-178C"}],
                "exploration_priority": "low",
            }
        )

        mock_chain = _make_chain_mock(entity_json)
        mock_prompt = MagicMock()
        mock_prompt.__or__ = MagicMock(return_value=mock_chain)
        ...
        with (
            ...
            await graph.ainvoke(
                {
                    "query": "What standards apply to aerospace?",
                    "context": "DO-178C is a standard for airborne software.",
                }
            )

        tracker = get_global_cost_tracker()
        calls = [c for c in tracker.get_calls() if c.operation == "entity_identification"]
        assert len(calls) == 1
        assert calls[0].model == mock_config.chat_model
        assert calls[0].input_tokens == 100
        assert calls[0].output_tokens == 50
